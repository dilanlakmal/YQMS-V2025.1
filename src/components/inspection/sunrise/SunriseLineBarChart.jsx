...
//   // Calculate the maximum defect rate and add 2% for padding
//   const maxDefectRate =
//     defectData.length > 0
//       ? Math.max(...defectData.map((d) => d.defectRate))
//       : 0;
//   const yAxisMax = maxDefectRate + 2; // Add 2% to the highest defect rate
...
//       y: {
//         beginAtZero: true,
//         max: yAxisMax, // Set the Y-axis maximum to the highest defect rate + 2%
...
const SunriseLineBarChart = ({ filteredData, filters }) => {
  ...
  const defectRateByGroup = () => {
    ...
    // Aggregate data by the selected group
    filteredData.forEach((row) => {
      // Determine the key based on the selected grouping
      let key;
      if (groupBy === "WorkLine") {
      ...
      if (!groupData[key]) {
        groupData[key] = {
          checked: 0,
          totalDefects: 0,
          reworkNameDefects: 0 // For the selected ReworkName
        };
      }

      const group = groupData[key];
      group.checked += row.CheckedQty;
      group.totalDefects += row.DefectsQty;

      // Calculate defects for the selected ReworkName (if any)
      if (filters.reworkName) {
        const matchingDefects = row.DefectDetails.filter(
    ...
    // Calculate defect rates for both total and ReworkName-specific defects
    return Object.entries(groupData)
      .map(([group, data]) => ({
        group,
        totalDefectRate:
        ...
        reworkNameDefectRate: filters.reworkName
          ...
          : null // Only calculate if a ReworkName is selected
      }))
  ...
  // Calculate the maximum defect rate for the Y-axis (considering both datasets)
        ...
        ? [
            {
              ...
              backgroundColor: defectData.map((d) =>
                d.reworkNameDefectRate > 5
                  ? "rgba(255, 99, 132, 0.6)" // Different color for ReworkName-specific rate
