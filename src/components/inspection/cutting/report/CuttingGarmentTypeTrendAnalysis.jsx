...
// import TrendTable from "./trends/TrendTable"; // Import the sub-component
...
// const GARMENT_TYPES_FOR_DISPLAY = [
//   "Top",
//   "Bottom",
//   "Zipper Jacket",
//   "Top Layer",
...
//   const getAQLResultStatus = (
//     totalInspectionQty,
//     sumTotalReject,
//     noOfInspections,
//     totalPcsForAQLDecision
//   ) => {
//     if (totalPcsForAQLDecision < totalInspectionQty) {
//       return {
//         key: "pending",
...
//           : null,
//         moNo: currentFilters.moNo || null,
//         tableNo: currentFilters.tableNo || null,
//         buyer: currentFilters.buyer || null,
//       };
//       try {
//         const [moRes, tableResConditional, buyerRes, garmentTypeRes] =
//           await Promise.all([
//             axios.get(`${API_BASE_URL}/api/cutting/filter-options/mo-numbers`, {
//               params: { ...params, search: moNoSearch },
...
//               params: { ...params, search: buyerSearch },
//               withCredentials: true,
//             }),
//             axios.get(
...
//       try {
//         const [garmentTypeRes, topMeasIssuesRes, topDefectIssuesRes] =
//           await Promise.all([
//             axios.get(`${API_BASE_URL}/api/cutting/trend/garment-type`, {
//               params,
//               withCredentials: true,
//             }),
//             axios.get(
...
//           // Only fetch sub-trends if main filters are applied
//           await fetchMeasurementPointTrend(
//             currentFilters,
//             trendGarmentTypeFilter,
//             trendPartNameFilter
//           );
//           await fetchFabricDefectTrend(
//             currentFilters,
//             trendGarmentTypeFilter,
//             trendPartNameFilter
//           );
...
//       }
//     },
//     [t, trendGarmentTypeFilter, trendPartNameFilter]
//   ); // Add trend filters

//   const fetchMeasurementPointTrend = async (
//     mainFilters,
//     garmentType,
//     partName
//   ) => {
...
//       moNo: mainFilters.moNo,
//       tableNo: mainFilters.tableNo,
//       buyer: mainFilters.buyer,
//       garmentType,
//       partName,
//     };
//     try {
...
//   const fetchFabricDefectTrend = async (mainFilters, garmentType, partName) => {
...
//   useEffect(() => {
//     fetchData(filters, true); // Initial fetch for all data based on default filters
...
//       );
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [trendGarmentTypeFilter, trendPartNameFilter, filters]); // Re-fetch if main filters change too
...
//       );
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [trendGarmentTypeFilter, trendPartNameFilter, filters]);
...
//     if (name === "moNo") {
//       setMoNoSearch(value);
//       if (!value) setTableNoOptions([]);
//     } // Clear table options if MO cleared
...
//         {" "}
//         {/* Changed to max-w-full for wider dashboard */}
...
//             { label: t("cutting.totalPcs"), subHeaders: ["T", "M", "B"] },
//             { label: t("cutting.reject"), subHeaders: ["T", "M", "B"] },
//             {
//               label: t("cutting.rejectMeasurements"),
//               subHeaders: ["T", "M", "B"],
//             },
//             { label: t("cutting.rejectDefects"), subHeaders: ["T", "M", "B"] },
//             { label: t("cutting.passRate"), subHeaders: ["T", "M", "B"] },
...
//               pending: 0,
//             };
//             const totalAQLInspections =
//               aqlSummary.pass + aqlSummary.reject + aqlSummary.pending;
//             const aqlPassRate =
...
//                       <span>{item.totalPcs?.top || 0}</span>
//                       <span>{item.totalPcs?.middle || 0}</span>
//                       <span>{item.totalPcs?.bottom || 0}</span>
//                     </div>
//                   </td>
//                   <td className="px-3 py-2 text-center border-r">
//                     <div className="grid grid-cols-3 gap-1">
//                       <span>{item.totalReject?.top || 0}</span>
//                       <span>{item.totalReject?.middle || 0}</span>
//                       <span>{item.totalReject?.bottom || 0}</span>
//                     </div>
//                   </td>
//                   <td className="px-3 py-2 text-center border-r">
//                     <div className="grid grid-cols-3 gap-1">
//                       <span>{item.totalRejectMeasurements?.top || 0}</span>
//                       <span>{item.totalRejectMeasurements?.middle || 0}</span>
//                       <span>{item.totalRejectMeasurements?.bottom || 0}</span>
//                     </div>
//                   </td>
//                   <td className="px-3 py-2 text-center border-r">
//                     <div className="grid grid-cols-3 gap-1">
//                       <span>{item.totalRejectDefects?.top || 0}</span>
//                       <span>{item.totalRejectDefects?.middle || 0}</span>
//                       <span>{item.totalRejectDefects?.bottom || 0}</span>
//                     </div>
//                   </td>
//                   <td className="px-3 py-2 text-center border-r">
//                     <div className="grid grid-cols-3 gap-1">
//                       <span>{(item.passRate?.top || 0).toFixed(0)}%</span>
//                       <span>{(item.passRate?.middle || 0).toFixed(0)}%</span>
//                       <span>{(item.passRate?.bottom || 0).toFixed(0)}%</span>
//                     </div>
//                   </td>
//                   <td className="px-3 py-2 text-left border-r text-xxs">
//                     <div className="flex items-center">
//                       <CheckCircle className="h-2.5 w-2.5 text-green-500 mr-0.5" />
//                       {t("common.pass")}: {aqlSummary.pass}
//                     </div>
//                     <div className="flex items-center">
//                       <XCircle className="h-2.5 w-2.5 text-red-500 mr-0.5" />
//                       {t("common.fail")}: {aqlSummary.reject}
//                     </div>
//                     <div className="flex items-center">
//                       <HelpCircle className="h-2.5 w-2.5 text-gray-400 mr-0.5" />
//                       {t("common.pending")}: {aqlSummary.pending}
//                     </div>
//                   </td>
//                 </tr>
//                 <tr
...
//                   <td
//                     colSpan={10}
//                     className="px-3 py-1 text-right font-medium text-gray-600 border-r text-xxs"
//                   >
//                     {t("cutting.aqlPassRate")}:
//                   </td>
//                   <td className="px-3 py-1 text-center font-semibold text-blue-600 text-xxs">
//                     {aqlPassRate}%
//                   </td>
//                 </tr>
//               </React.Fragment>
...
//                 className="mt-1 w-full p-1.5 border border-gray-300 rounded-md shadow-sm text-xs"
//                 disabled={!trendGarmentTypeFilter}
//               >
//                 <option value="">{t("common.all")}</option>
...
//               <tr className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
//                 <td
//                   className="px-3 py-2 whitespace-normal border-r sticky left-0 bg-inherit z-5"
//                   rowSpan={2}
//                 >
//                   {getLocalizedText(
//                     item.garmentType,
...
//                 </td>
//                 <td
//                   className="px-3 py-2 whitespace-normal border-r sticky left-[6rem] bg-inherit z-5"
//                   rowSpan={2}
//                 >
//                   {getLocalizedText(
//                     item.partName,
...
//                 </td>
//                 <td
//                   className="px-3 py-2 whitespace-normal border-r sticky left-[13rem] bg-inherit z-5"
//                   rowSpan={2}
//                 >
//                   {getLocalizedText(
...
//                   <td
//                     key={date}
//                     className="px-3 py-2 text-center border-r text-xxs"
//                   >
...
//                 className={`${
//                   index % 2 === 0 ? "bg-white" : "bg-gray-50"
//                 } text-xxs font-semibold text-blue-600`}
//               >
//                 {(measurementPointTrendData.headers || []).map((date) => (
//                   <td
//                     key={`${date}-rate`}
//                     className="px-3 py-1 text-center border-r"
//                   >
//                     {(item.values[date]?.passRate || 0).toFixed(2)}%
//                   </td>
//                 ))}
//               </tr>
//             </React.Fragment>
...
//             (trendGarmentTypeFilter
//               ? `, ${t("cutting.panel")}: ${trendGarmentTypeFilter}`
//               : "") +
//             (trendPartNameFilter
//               ? `, ${t("cutting.partNameFilter")}: ${trendPartNameFilter}`
//               : "")
//           }
//           titleIcon={TrendingUp}
...
//                 </td>
//                 {(fabricDefectTrendData.headers || []).map((date) => (
//                   <td key={date} className="px-3 py-2 text-center border-r">
//                     {item.values[date]?.rejectCount || 0}
//                   </td>
//                 ))}
//               </tr>
//               <tr
//                 className={`${
//                   index % 2 === 0 ? "bg-white" : "bg-gray-50"
//                 } text-xxs font-semibold text-red-600`}
//               >
//                 {(fabricDefectTrendData.headers || []).map((date) => (
//                   <td
//                     key={`${date}-rate`}
//                     className="px-3 py-1 text-center border-r"
//                   >
//                     {(item.values[date]?.defectRate || 0).toFixed(2)}%
//                   </td>
//                 ))}
//               </tr>
//             </React.Fragment>
...
//             headers={[
//               { label: t("cutting.measurementPoint") },
//               { label: t("cutting.passPoints"), className: "text-center" },
//               {
...
//                 className: "text-center",
//               },
//               { label: t("cutting.issuePercentage"), className: "text-center" },
//             ]}
...
//               >
//                 <td className="px-3 py-2 whitespace-normal border-r">
//                   {getLocalizedText(
//                     item.measurementPoint,
//                     item.measurementPoint,
//                     item.measurementPoint
...
//             headers={[
//               { label: t("cutting.defectName") },
//               { label: t("cutting.defectQty"), className: "text-center" },
//               { label: t("cutting.defectRate"), className: "text-center" },
//             ]}
